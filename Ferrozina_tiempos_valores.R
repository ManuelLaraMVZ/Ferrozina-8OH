...
modelo <- lm(Valores~Tratamiento, data = datos_ferrozina2)
...
byf.shapiro(Valores~Tratamiento, data=datos_ferrozina2)

leveneTest(Valores~Tratamiento, data=datos_ferrozina2)
fligner.test(Valores~Tratamiento, data=datos_ferrozina2)
bartlett.test(Valores~Tratamiento, data=datos_ferrozina2)
...
ANOVA_dual<- aov( Valores~Tratamiento,data = datos_ferrozina2) #Ponemos de donde se sacan los datos y luego la relación
...
barras2 <- barras1+
  ...
  geom_text(aes(label=similitud),
            ...
            nudge_y =.0750,      #respecto al eje y que tanto cambia
            size=6,
            face="bold")+
  scale_fill_simpsons()+
